...
public class Woodcutter extends LoopingBot implements PaintListener {
    ...
    private Coordinate lastWillowTile;
    ...
    private void doFindTree() {
        ...
        if (currentTree != null && inWillowArea(currentTree.getPosition())) {
            lastWillowTile = currentTree.getPosition();
            log("Tree found at: " + lastWillowTile);
    ...
    private void doBank() {
        ...
        if (!Bank.isOpen()) {
            ...
            Player me = Players.getLocal();
            if (me != null && me.getPosition().distanceTo(DRAYNOR_BANK) <= 5) {
                Bank.open();
